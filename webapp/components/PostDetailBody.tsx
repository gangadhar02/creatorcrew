 ...
 * PostDetailModal (Dialog) and the Eden-style inset PostOverlay so both stay in
 * sync. The chrome (header actions, close) belongs to each container.
 */
export default function PostDetailBody({
  ...
  /** Background behind the media band. Black for the modal; transparent on the
   *  white inset overlay. */
  mediaBg?: string;
}) {
  ...
  // Resolve a directly-playable video URL.
  useEffect(() => {
    setVideoUrl(null);
      ...
      <div className={cn("flex items-center justify-center p-4", isX ? "bg-muted/30" : mediaBg)}>
        {videoUrl && !videoFailed ? (
          ...
          <iframe
            ...
            allowFullScreen
            scrolling="no"
            className="block w-full max-w-[400px] rounded-lg border-0 bg-white"
            style={{ height: 560 }}
          />
        ) : thumbnail ? (
